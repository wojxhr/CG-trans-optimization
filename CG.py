...
class CARL(BlockSelection,CongestionControl):
    '''
        级联的网络模型，先是一个deadline级别决策感知(DQN)。然后对状态输入进行抽取与attention，然后进行双重dqn决策
        注意要分开进行训练
        input:每个用户一段时间内控制信令的信息，与vmaf值
        output:每个用户的相对优先级
        '''

    def __init__(self):
        super(CARL, self).__init__()
        self.user_control_info = dict()
        self.vmaf_series=[]


        self.timeseries=nn.GRU(input_size=)
        self.DeadlineModel=DQN(N_STATES=USER_NUM,N_ACTIONS=USER_NUM)
...
class RL(CongestionControl):
    ...
    def estimate_bandwidth(self, cur_time, event_info):
        ...
        if self.counter == EPISODE:  # choose action every EPISODE times
            ...
            instant_rate = len(instant_ack_packet) / (instant_packet[-1][0] - instant_packet[0][0]) if (instant_packet[
                                                                                                            -1][0] -
                                                                                                        instant_packet[
                                                                                                            0][
                                                                                                            0]) > 0 else 0
