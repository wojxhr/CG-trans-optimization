...
class MySolution(BlockSelection, Reno):
    ...
    def cc_trigger(self, cur_time, event_info):
        ...
        left = self.sum_cwnd/self.counter
        right = self.min_cwnd + (self.max_cwnd-self.min_cwnd)/3
        if np.mean(self.cwnd_list) == self.max_cwnd:
            if (self.counter > 10):
                if left > right:
                    self.max_cwnd += 1
                # else:
                #     self.max_cwnd += 1
        elif np.mean(self.cwnd_list) == self.min_cwnd:
            self.min_cwnd -= 1
        elif np.mean(self.cwnd_list) < MAX_CWND:
            self.max_cwnd = MAX_CWND
        elif np.mean(self.cwnd_list) > MIN_CWND:
            self.min_cwnd = MIN_CWND
        ...
        if self.curr_state == self.states[0]:
            if self.rtt[-1] < self.rtt_base * 1.2:
                ...
                avg = (self.max_cwnd + self.min_cwnd)//2
                if self.cwnd > avg:
                    self.cwnd = min(self.max_cwnd, self.cwnd+1)
                else:
                    self.cwnd = min(self.max_cwnd, self.cwnd+2)
                # self.cwnd = min(self.max_cwnd, self.cwnd+(self.max_cwnd-self.cwnd)/4)
