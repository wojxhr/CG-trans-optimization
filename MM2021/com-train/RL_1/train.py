...
N_F = 9  # speed,losepacket,application_speed
N_A = 7  # +100,0,-100
A_list = [100, 300, 500, 800, 1000, 1500, 2500]
MAX_BANDWITH = 2500  # standardlize to 1
P = [1, 2/3, 1/3]
NN_MODEL = None
...
class RL(CongestionControl):
    ...
    def estimate_bandwidth(self, cur_time, data):
        ...
        if self.counter == EPISODE:  # choose action every EPISODE times
            ...
            # print(rtt * self.send_rate - inflight)
            ...
            state[1] = thr_use / thr_real
            state[2] = np.mean(self.rtt) * 10
            state[3] = (self.rtt[-1] - self.rtt[-5]) * 100
            state[4] = self.instant_drop_nums / EPISODE
            state[5] = (self.last_inflight[-1] - self.last_inflight[0])/(time_interval + 0.0001) / (self.send_rate - self.last_send_rate + 1) / 2500
            state[6] = (self.last_inflight[-1] - self.last_inflight[-5]) / 10
            state[7] = self.last_inflight[-1] / (self.last_inflight[0] + 1)
            state[8] = (self.last_inflight[-1] - self.last_inflight[-3]) / (self.last_inflight[-3] - self.last_inflight[-5] + 0.1)
            ...
            self.reward = 0.4 * self.send_rate / 2500 + 0.3 * thr_use / thr_real - 5 * self.instant_drop_nums / EPISODE - 0.5 * abs(self.send_rate - self.last_send_rate)/2500
            self.last_send_rate = self.send_rate
            self.send_rate = A_list[int(a)]
