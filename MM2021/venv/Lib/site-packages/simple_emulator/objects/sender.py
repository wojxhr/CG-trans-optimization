...
class Sender():
    ...
    def __init__(self, path, dest, features, history_len=10, solution=None):
        ...
        # logged for QoE
        self.H_recv_packet=0
        self.M_recv_packet=0
        self.L_recv_packet=0

        self.H_loss_packet = 0
        self.M_loss_packet = 0
        self.L_loss_packet = 0
    ...
    def init_application(self, block_file,**kwargs):
        ...
        self.application = AppicationLayer(block_file, bytes_per_packet=BYTES_PER_PACKET,sender_id=self.id ,**kwargs)
    ...
    def on_packet_acked(self, rtt, packet, event_time):
        ...
        """
        self.acked += 1
        if packet.block_info["Priority"] == 0:
            self.H_recv_packet+=1
        elif packet.block_info["Priority"] == 1:
            self.M_recv_packet+=1
        elif packet.block_info["Priority"] == 2:
            self.L_recv_packet+=1
        else:
            print("WRONG Priority!!!")
            exit(1)
        diff=rtt-self.last_rtt
        self.jitter.append(diff)
    ...
    def on_packet_lost(self, event_time, packet):
        ...
        self.lost += 1
        if packet.block_info["Priority"] == 0:
            self.H_loss_packet+=1
        elif packet.block_info["Priority"] == 1:
            self.M_loss_packet+=1
        elif packet.block_info["Priority"] == 2:
            self.L_loss_packet+=1
        else:
            print("WRONG Priority!!!")
            exit(1)
    ...
    def calculate_info(self):
        # avg_loss=dict()
        # avg_loss["H_avg_loss"]=float(self.H_loss_packet / (self.H_loss_packet+self.H_recv_packet))
        # avg_loss["M_avg_loss"] = float(self.M_loss_packet / (self.M_loss_packet + self.M_recv_packet))
        #
        # jitter=numpy.mean(self.jitter)
        # with open("./loss_info.log","a") as f:
        #     f.write(json.dumps(avg_loss)+ '\n')
        # with open("./jitter_info.log","a") as f:
        #     f.write(str(jitter)+'\n')
        pass