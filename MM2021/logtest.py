...
class MySolution(BlockSelection, Reno):
    ...
    def cc_trigger(self, cur_time, event_info):
        ...
        if self.index % 5 == 0:
            ...
            if np.mean(self.instant_drop_nums_list[-5:]) < 2 and self.cwnd < MAX_CWND/2:
                self.cwnd = min(self.max_cwnd, self.cwnd * 1.5)

            if np.mean(self.instant_drop_nums_list) > 3 and self.cwnd == MIN_CWND:
                self.cwnd = MIN_CWND-10

        if self.instant_drop_nums == 0 and self.cwnd<MIN_CWND:
            self.cwnd=MIN_CWND

        if self.instant_drop_nums > 3:
            self.cwnd=40
