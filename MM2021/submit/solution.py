...
class MySolution(BlockSelection, Reno):
    ...
    def select_block(self, cur_time, block_queue):
        ...
        def is_better(block):
            ...
            best_block_ddl_ratio = best_block_rest_pkt / (best_block_ddl_rest + 0.0001)
            block_ddl_radio = block_rest_pkt /  (block_ddl_rest + 0.0001)
            # print(best_block_ddl_ratio)
            ...
            #     return False
            # all information
            if best_block_ddl_ratio * best_block_rest_ratio * P[best_block_priority] > block_ddl_radio * block_rest_ratio * P[cur_block_priority]:
                return True
            else:
                return False
    ...
    def cc_trigger(self, cur_time, event_info):
        ...
        # if packet is acknowledged
            ...
            if self.curr_state == self.states[0]:
                if self.ack_nums == self.cwnd:
                    ...
                    self.cwnd = min(self.max_cwnd, self.cwnd * 4)
                    self.ack_nums = self.ack_nums

            # increase cwnd linearly in congestion_avoidance state
            elif self.curr_state == self.states[1]:
                if self.rtt[-2] - self.rtt[-1] > self.rtt_base * 2:
                    self.cwnd = self.cwnd * 1.4
                if self.ack_nums == self.cwnd and (float(self.rtt[-1]-self.rtt_base)/self.rtt_base) < 0.53:
                    self.cwnd = min(self.max_cwnd, self.cwnd + 7)
                    self.ack_nums = 0

        # reset threshhold and cwnd in fast_recovery state
        if self.curr_state == self.states[2]:
            # self.ssthresh = max(self.min_cwnd, self.cwnd // 1.01)
            # self.cwnd = self.ssthresh
            # self.curr_state = self.states[1]
            max_min_window = self.max_cwnd - self.min_cwnd
            if self.ssthresh >= self.min_cwnd + max_min_window // 2.3:
                self.ssthresh = max(self.min_cwnd, self.cwnd // 1.01)
            else:
                self.ssthresh = max(self.min_cwnd, self.cwnd // 1.05)
            self.cwnd = self.ssthresh
            self.curr_state = self.states[1]
        ...
        #     line = str(cur_time) + ',' + str(self.cwnd) + ',' + str(rtt) + ',' + str(inflight) + '\n'
